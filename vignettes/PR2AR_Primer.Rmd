...
output:
  ...
  pdf_document:
    keep_tex: yes
vignette: "%\\VignetteIndexEntry{PR2AR_Primer} %\\VignetteEngine{knitr::rmarkdown}
  %\\VignetteEncoding{UTF-8} % \\VignetteDepends{data.table, ggplot2} \n"
---
...
knitr::opts_chunk$set(
  ...
  comment = "#>",
  echo = TRUE)
library(data.table); library(ggplot2); library(DiagrammeR)
```
...
\begin{equation} 
X_{t+1} = A_t \bar{X_t} + Q X_t  
\end{equation}
where $\bar{x} \equiv 1 - x$. From this, we can solve for $A_t$: 
\begin{equation} 
A_t = \frac{X_{t+1} - Q X_t}{\bar{X_t}}
\end{equation}
At the steady state, $X_{t+1} = X_t,$ so we get the relationship: 
$$\frac{A}{\bar{Q}} = \frac{X}{\bar{X}}$$
Note that $A$, a probability, can be at most 1, so $X \leq 1/(2-Q)$. If we take a 14-day time step and assume $r = \frac{1}{200}$ (a 200 day average length of infection), then we get that Equation 2 gives nonsensical results for $X > 0.93$. Since there is no evidence that being infected protects against future infection, this is an artifact. 
...
## A Better Start
We don't want to work around artifacts, so what if we rethink the model. What if anyone can get infected, including the fraction that would have otherwise cleared their infections? The susceptible class is $1-X_t + \bar{Q}X_t$, or more simply: 
\begin{equation} 
X_{t+1} = A_t (1 - Q) X_t + Q X_t  
\end{equation}
This represents multiple events occurring in a time step -- recovery followed by infection. These events are ordered, with recovery coming first, leading to the additional term appearing in the update equation for the infected class. Now, when $A=1$, at the steady state, we get that $X=1$. 
...
S_{t+1} &=  \bar{A_t} S_t +  \bar{A_t}\bar{Q}X_t\\
X_{t+1} &=  A_t S_t + A_t\bar{Q}X_t  + Q X_t \\
...
-A & \bar{A}\bar{Q} \\
A &  -\bar{A}\bar{Q}\\
\end{array} \right] \cdot Y = A S - \bar{A}\bar{Q}X = 0
$$
...
$$A^* = \mathrm{arg\,min}_{A \in [0,1]} f(A) = |X - D Y_{eq}(A)|$$
