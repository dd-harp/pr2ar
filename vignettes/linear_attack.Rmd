...
plotirange <- function() {
  ...
  legend(
    .5, .8,
    legend = c("peekahead", "exact master equation", "single jump"),
...
plotinverse <- function() {
  ...
  lines(pr, picardrelerr, col = "blue")
  legend(
    .3, .4,
    legend = c("peekahead", "single jump", "picard"),
...
plotirange <- function() {
  ...
  legend(
    .6, .8,
    legend = c("peekahead", "picard", "exact master equation", "single jump"),
    col = c("green", "orange", "red", "purple"),
    lty = 1)
}
plotirange()
```
...
$$
You can mitigate this error, converting to to $O(h^2)$,
by rewriting using Strang splitting [@celledoni2001methods].
$$
...
If we take the susceptible-infected-treated example, what does Strang
splitting look like? Define $s = e^{-ht/2}$.
$$
...
$$
  e^{tA} = e^{\lambda_1t}I+\sum_{j=2}^n[\lambda_1,\ldots,\lambda_j]\prod_{k=1}^{j-1}(A-\lambda_kI)
$$
...
[\lambda_1,\ldots,\lambda_{k+1}] & = & \frac{[\lambda_1,\ldots,\lambda_k]-[\lambda_2,\ldots,\lambda_{k+1}]}{\lambda_1-\lambda_{k+1}}
\end{eqnarray}
