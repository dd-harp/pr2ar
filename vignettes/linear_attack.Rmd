...
As a reminder, it would be excellent to write the time step so that we can
solve directly for the attack rate. That means that some part of the matrix
that does updates is linear in $A$.
$$
  x = (M_0 + AM_1) x_0
$$
When we have that structure, as we do with the peekahead, then we can
"read out" the attack rate from the prevalence.
