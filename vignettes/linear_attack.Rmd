...
\end{array}\right]
$$
Let's check that with some numbers
```{r peek_around}
...
a_1 e^{-rt}\left[\begin{array}{c}1 \\ (h-r)/r \\ -h/r\end{array}\right] +
a_2 \left[\begin{array}{c}0 \\ 0 \\ 1\end{array}\right]
$$
Initial values are $(1-I, I, 0)$, so $a_2=0$. This leads to
$$
\left[\begin{array}{c}S \\ I \\ \tilde{I}\end{array}\right] =
(1-\frac{h}{h-r}I_0) e^{-ht}\left[\begin{array}{c}1 \\ 0 \\ 1\end{array}\right] +
\frac{r}{h-r}I_0 e^{-rt}\left[\begin{array}{c}1 \\ (h-r)/r \\ -h/r\end{array}\right] +
$$
If we write this out by component, we see $I = I_0e^{-rt}$ and the other $I$ is
$$
\tilde{I} = (1-\frac{h}{h-r}I_0)e^{-ht} + \frac{-h}{h-r}I_0e^{-rt}
$$
Together, they give
$$
P[I] = I + \tilde{I} = (1-\frac{h}{h-r}I_0)e^{-ht} + \frac{-r}{h-r}I_0e^{-rt}
$$
...
# Derive the Better Equation

Can we derive the peekahead method froma master equation?
This would mean we write a master equation that allows the
$I\rightarrow S\rightarrow I$ transition and solve it.
